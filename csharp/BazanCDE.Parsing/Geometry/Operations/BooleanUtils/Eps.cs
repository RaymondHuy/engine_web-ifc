namespace BazanCDE.Parsing.Operations.BooleanUtils
{
    public static class Eps
    {
        // Matches C++ inline globals in boolean-utils/eps.h (mutable at runtime).
        public static double _TOLERANCE_PLANE_INTERSECTION = 1.0E-04;
        public static double _TOLERANCE_PLANE_DEVIATION = 1.0E-04;
        public static double _TOLERANCE_BACK_DEVIATION_DISTANCE = 1.0E-04;
        public static double _TOLERANCE_INSIDE_OUTSIDE_PERIMETER = 1.0E-10;
        public static double _TOLERANCE_BOUNDING_BOX = 1.0E-02;

        // Debug value.
        public static double _BOOLSTATUS = 1.0E-10;

        public const bool messages = false;

        /*
            Constants from the original eps.h. However,
                EPS_TINY, EPS_SMALL, EPS_BIG, EPS_BIG2 and SCALED_EPS_BIG
            have been assigned a new, common value.
        */
        public const double EPS_NONZERO = 1.0E-20;
        public const double EPS_MINISCULE = 1.0E-12;
        public const double EPS_TINY = 1.0E-04;
        public const double EPS_SMALL = 1.0E-04;
        public const double EPS_BIG = 1.0E-04;
        public const double EPS_BIG2 = 1.0E-03;
        public const double SCALED_EPS_BIG = 1.0E-04;

        /*
            Constants used in math.h
        */
        public const double toleranceIsInsideCenterExtents = 1.0E-10;

        /*
            Constants used in shared-position.h
        */
        public const double toleranceVectorEquality = 1.0E-04;
        public const double TOLERANCE_SCALAR_EQUALITY = 1.0E-04;
        public const double toleranceCollinear = 1.0E-04;
        public const double triangleEvaluationFactor = 0.90;

        /*
            Constants used in shared-position
        */
        public const double tolerancePointOnLine = 1.0E-04;

        /*
            Constants used in aabb.h
        */
        public const double toleranceAABB = 1.0E-03;

        /*
            Constants used in function intersect_ray_triangle
        */
        public const double toleranceParallel = 1.0E-04;
        public const double toleranceParallelTight = 1.0E-10;
        public const double toleranceThinTriangle = 1.0E-10;

        /*
            Constants used in function AddFace in geometry.h
        */
        public const double toleranceAddFace = 1.0E-10;

        /*
            Round all coordinates when they are read to a specified number of decimal places.
            To disable/enable rounding, set ROUNDING_ENABLE to 0/1.
        */
        public const int ROUNDING_ENABLE = 0;
        public const double ROUNDING = 1.0E-04;
        public const double ROUNDING_RECIPROCAL = 1.0E+04;
    }
}
